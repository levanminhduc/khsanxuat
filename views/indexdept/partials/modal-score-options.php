        <div id="scoreOptionsModal" class="modal score-options-modal indexdept-modern-modal tw-fixed tw-inset-0 tw-z-[10000] tw-hidden tw-items-center tw-justify-center tw-overflow-y-auto tw-bg-slate-950/55 tw-p-4 tw-backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="score-options-title" hidden>
            <div class="modal-content default-settings-modal__content score-options-modal__content tw-relative tw-m-0 tw-flex tw-max-h-[92vh] tw-w-full tw-max-w-6xl tw-flex-col tw-overflow-hidden tw-rounded-2xl tw-border tw-border-white/70 tw-bg-white tw-p-0 tw-shadow-modal-soft">
                    ...
                    <div class="tw-flex tw-items-start tw-justify-between tw-gap-4">
                        <div class="score-options-modal__heading tw-flex tw-min-w-0 tw-items-start tw-gap-4">
                            ...
                            <div class="tw-min-w-0">
                                ...
                                <h3 id="score-options-title" class="modal-title default-settings-modal__title score-options-modal__title tw-m-0 tw-mt-1 tw-pr-12 tw-text-2xl tw-font-bold tw-leading-tight tw-text-slate-950">
                                    Cài mốc điểm
                                </h3>
                            </div>
                        </div>
                ...
                <div id="score_options_status" class="default-settings-modal__status score-options-modal__status tw-mx-6 tw-mt-4" hidden></div>
                ...
                <div class="default-settings-modal__body score-options-modal__body tw-flex tw-min-h-0 tw-flex-1 tw-flex-col tw-gap-4 tw-px-6 tw-py-5">
                        ...
                        <table class="evaluation-table default-settings-modal__table score-options-modal__table tw-w-full tw-border-collapse tw-text-sm">
                            <thead class="tw-sticky tw-top-0 tw-z-10">
                                <tr class="tw-bg-slate-100 tw-text-left tw-text-xs tw-font-bold tw-uppercase tw-tracking-wide tw-text-slate-600">
                                    <th class="score-options-modal__col score-options-modal__col--stt tw-px-4 tw-py-3">STT</th>
                                    <th class="score-options-modal__col score-options-modal__col--criteria tw-px-4 tw-py-3">Tiêu chí đánh giá</th>
                                    <th class="score-options-modal__col score-options-modal__col--state tw-px-4 tw-py-3">Trạng thái</th>
                                    <th class="score-options-modal__col score-options-modal__col--scores tw-px-4 tw-py-3">Mốc điểm</th>
                                    <th class="score-options-modal__col score-options-modal__col--actions tw-px-4 tw-py-3">Thao tác</th>
                                </tr>
                            </thead>
                            <tbody id="score_options_tbody" class="tw-divide-y tw-divide-slate-100">
                                ...
                                while ($row = $result->fetch_assoc()) {
                                    if ($current_nhom != $row['nhom']) {
                                        ...
                                        ?>
                                        <tr>
                                            <td colspan="5" class="default-settings-modal__group-row tw-bg-slate-900 tw-px-4 tw-py-2 tw-text-left tw-text-xs tw-font-bold tw-uppercase tw-tracking-wide tw-text-slate-700">
                                    ...
                                    <tr id="score_row_<?php echo $id_tieuchi; ?>"
                                        ...
                                        <td class="score-options-modal__index tw-whitespace-nowrap tw-px-4 tw-py-3 tw-font-bold tw-text-slate-700"><?php echo (int) $row['thutu']; ?></td>
                                        <td class="text-left score-options-modal__criteria-cell tw-min-w-[320px] tw-px-4 tw-py-3 tw-leading-5 tw-text-slate-800"><?php echo htmlspecialchars($row['noidung']); ?></td>
                                        <td class="score-options-modal__state-cell tw-px-4 tw-py-3">
                                            <span id="score_badge_<?php echo $id_tieuchi; ?>"
                                                  class="score-options-modal__badge <?php echo $has_custom_scores ? 'score-options-modal__badge--custom tw-border-[#b8cdf4] tw-bg-[#eef5ff] tw-text-[#143583]' : 'score-options-modal__badge--default tw-border-slate-200 tw-bg-slate-50 tw-text-slate-700'; ?> tw-inline-flex tw-h-8 tw-items-center tw-rounded-full tw-border tw-px-3 tw-text-xs tw-font-bold">
                                                <?php echo $has_custom_scores ? 'Tùy chỉnh' : 'Mặc định'; ?>
                                            </span>
                                        </td>
                                        <td class="score-options-modal__input-cell tw-px-4 tw-py-3">
                                            <input type="text"
                                                   id="score_values_<?php echo $id_tieuchi; ?>"
                                                   class="form-control score-options-modal__score-input tw-h-10 tw-min-w-[150px] tw-rounded-lg tw-border-slate-300 tw-bg-white tw-text-sm tw-shadow-sm focus:tw-border-[#143583] focus:tw-ring-4 focus:tw-ring-[#143583]/15"
                                                   value="<?php echo htmlspecialchars($score_values); ?>"
                                                   placeholder="0, 1, 3"
                                                   inputmode="decimal"
                                                   aria-label="Mốc điểm cho tiêu chí <?php echo (int) $row['thutu']; ?>">
                                        </td>
                                        <td class="score-options-modal__action-cell tw-px-4 tw-py-3">
                                            <div class="score-options-modal__row-actions tw-flex tw-gap-2">
                    ...
                    <div class="default-settings-modal__toolbar default-settings-modal__toolbar--bottom score-options-modal__toolbar score-options-modal__toolbar--bottom tw-flex tw-flex-col-reverse tw-gap-3 tw-border-t tw-border-slate-100 tw-pt-4 sm:tw-flex-row sm:tw-justify-end">
                        <button type="button" onclick="closeScoreOptionsModal()" class="btn-add-criteria default-settings-modal__btn default-settings-modal__btn--secondary score-options-modal__btn score-options-modal__btn--secondary tw-inline-flex tw-h-11 tw-items-center tw-justify-center tw-rounded-xl tw-border tw-border-slate-300 tw-bg-white tw-px-5 tw-text-sm tw-font-bold tw-text-slate-700 tw-shadow-sm tw-transition hover:tw-bg-slate-50">
