...
<head>
    ...
    <style>
        ...
        /* Cải thiện modal cho điện thoại */
        .modal {
            display: none;
            position: fixed;
            z-index: 1000;
...
<body>
        ...
        <div class="card">
            ...
            <div class="gallery">
                ...
                <div class="gallery-item">
                    ...
                    <div class="overlay">
                        <a href="#" class="js-open-modal" data-src="<?php echo $image['image_path']; ?>" 
                           data-tieuchi="<?php echo htmlspecialchars($image['tieuchi_name'] ?? 'Không xác định'); ?>" 
                           data-date="<?php echo date('d/m/Y H:i', strtotime($image['upload_date'])); ?>">Xem</a>
    ...
    <div id="imageModal" class="modal">
        ...
        <div class="modal-navigation">
            <div class="nav-btn prev-btn">&lsaquo;</div>
            <div class="nav-btn next-btn">&rsaquo;</div>
        </div>
    ...
    // Lưu trữ tất cả các ảnh và thông tin của chúng
    const allImages = [
        <?php foreach ($images as $index => $image): ?>
        {
            src: "<?php echo $image['image_path']; ?>",
            tieuchi: "<?php echo addslashes($image['tieuchi_name'] ?? 'Không xác định'); ?>",
            uploadDate: "<?php echo date('d/m/Y H:i', strtotime($image['upload_date'])); ?>"
    ...
    // Tạo thumbnails cho tất cả các ảnh
    ...
    function displayCurrentImage() {
        ...
        modalInfo.innerHTML = `<strong>Tiêu chí:</strong> ${image.tieuchi}<br><strong>Ngày upload:</strong> ${image.uploadDate}`;
    ...
    function openModal(imgSrc, tieuchi, uploadDate) {
        ...
        // Tìm index của ảnh đang được mở
        ...
        // Preload ảnh trước và ảnh tiếp theo
        if (allImages.length > 1) {
            const nextIndex = (currentImageIndex + 1) % allImages.length;
            const prevIndex = (currentImageIndex - 1 + allImages.length) % allImages.length;
            new Image().src = allImages[nextIndex].src;
            new Image().src = allImages[prevIndex].src;
        }
    }
    ...
    // Sửa lỗi hiển thị modal trên iOS
    ...
    // Sửa lại cách gán sự kiện click cho nút "Xem"
    document.addEventListener('DOMContentLoaded', function() {
        // Gán sự kiện cho tất cả các nút xem ảnh
        const viewButtons = document.querySelectorAll('.js-open-modal');
        viewButtons.forEach(button => {
            button.addEventListener('click', function(e) {
                e.preventDefault(); // Ngăn hành vi mặc định của thẻ a
                const imgSrc = this.getAttribute('data-src');
                const tieuchi = this.getAttribute('data-tieuchi');
                const uploadDate = this.getAttribute('data-date');
                openModal(imgSrc, tieuchi, uploadDate);
            });
        });
    });
    </script>
